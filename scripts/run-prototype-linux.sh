...
need_cmd() {
  if ! command -v "$1" >/dev/null 2>&1; then
    ...
    case "$1" in
      cargo)
        echo "On Ubuntu/Debian: install Rust from https://rustup.rs/" >&2
        ;;
      curl | unzip)
        echo "On Ubuntu/Debian: sudo apt install curl unzip" >&2
        ;;
      xdg-open)
        echo "On Ubuntu/Debian: sudo apt install xdg-utils" >&2
        ;;
    esac
    exit 1
  fi
...
if [[ ! -f "$VENDOR_DIR/lib/libvosk.so" ]]; then
  ...
  echo "Native Vosk library already installed at $VENDOR_DIR/lib/libvosk.so"
fi
